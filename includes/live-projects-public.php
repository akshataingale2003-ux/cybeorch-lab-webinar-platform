...
function lpProjectRoot(): string
{
    return defined('CYBEORCH_APP_ROOT') ? CYBEORCH_APP_ROOT : dirname(__DIR__);
}

/** Absolute path to live project image folder. */
function lpLiveProjectsImageDir(): string
{
    return lpProjectRoot() . '/assets/images/live_projects';
}

function lpAssetExists(string $relativePath): bool
{
    $relativePath = str_replace('\\', '/', ltrim($relativePath, '/'));
    $fsPath = lpProjectRoot() . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $relativePath);
    return is_file($fsPath);
}

/**
 * Canonical slug -> image filename in assets/images/live_projects/.
 *
 * @return array<string, string>
 */
function lpLiveProjectSlugImageMap(): array
{
    return [
        'ai-and-ml'                                        => 'AI & ML1.png',
        'blockchain-technology'                            => 'blockchain_technology.png',
        'buy-credits-for-shopping-system'                  => 'Buy Credits for Shopping System.png',
        'cafe-merchant-redemption-pos-bridge'              => 'Cafe Merchant Redemption & POS Bridge.png',
        'cyber-security'                                   => 'Cybersecurity.png',
        'data-bases-secutity'                              => 'data bases secutity.png',
        'data-science'                                     => 'Data Science.png',
        'database-security-cryptographic-protection-suite'   => 'Database Security & Cryptographic Protection Suite.png',
        'fintech'                                          => 'FinTech.png',
        'java-full-stack-development'                      => 'Java Full Stack Development.png',
        'mern-full-stack-development'                      => 'MERN Full Stack Development.png',
        'own-nxl-credits-tokens-system'                    => 'Own NXL Credits Tokens System.png',
        'payment-gateway'                                  => 'payment gateway.png',
        'token-creation'                                   => 'Token creation.png',
        'web3-crypto-wallet-creation'                      => 'Web3 Crypto Wallet Creation.png',
        'web3-wallet-security-cryptographic-protection'    => 'Web3 Wallet Security & Cryptographic Protection.png',
    ];
}

/**
 * Resolve image path (relative) from assets/images/live_projects/.
 *
 * @param array<string, mixed> $project
 */
function liveProjectResolvedImagePath(array $project): string
{
    $base = 'assets/images/live_projects/';
    $slug = strtolower(trim((string) ($project['slug'] ?? '')));
    $stored = str_replace('\\', '/', trim((string) ($project['image_path'] ?? '')));

    if ($stored !== '' && str_contains($stored, 'assets/images/live_projects/') && lpAssetExists($stored)) {
        return $stored;
    }

    $slugMap = lpLiveProjectSlugImageMap();
    if ($slug !== '' && isset($slugMap[$slug])) {
        $path = $base . $slugMap[$slug];
        if (lpAssetExists($path)) {
            return $path;
        }
    }

    $title = strtolower(trim((string) ($project['title'] ?? '')));
    $dir = lpLiveProjectsImageDir();
    if ($title !== '' && is_dir($dir)) {
        foreach (scandir($dir) ?: [] as $file) {
            ...
            if ($stem === $title || str_contains($stem, $title) || str_contains($title, $stem)) {
                return $base . $file;
            }
        }
    }

    foreach (array_values($slugMap) as $filename) {
        $path = $base . $filename;
        if (lpAssetExists($path)) {
            return $path;
        }
...
{
    return liveProjectResolvedImagePath($project);
}
...
function liveProjectImageUrl(array $project): string
{
    ...
    return lpAssetUrl(liveProjectResolvedImagePath($project));
}
...
function renderLiveProjectsPageStyles(): void
{
    echo '<style>'
        ...
        . '.lp-meta{display:flex;flex-wrap:nowrap;gap:.5rem .85rem;font-size:.78rem;color:var(--cyber-muted);margin-bottom:.85rem;align-items:center;}'
...
function renderLiveProjectCard(array $project, array $opts = []): void
{
    ...
    <article class="lp-card<?= $wide ? ' lp-card--wide' : '' ?>">
      <div class="lp-card-media <?= htmlspecialchars($theme) ?>">
        <img src="<?= htmlspecialchars($imageUrl) ?>" alt="<?= htmlspecialchars((string) ($project['title'] ?? 'Live project')) ?>" loading="lazy" decoding="async" style="object-fit:cover">
