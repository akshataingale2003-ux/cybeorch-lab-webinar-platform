...
function adminPaymentsFilterClause(
    ...
    if ($searchQ !== '') {
        $parts[] = '(u.full_name LIKE ? OR u.email LIKE ? OR u.phone LIKE ? OR p.order_id LIKE ? OR p.razorpay_order_id LIKE ? OR p.razorpay_payment_id LIKE ? OR p.invoice_no LIKE ? OR p.receipt_no LIKE ? OR p.nxl_transaction_ref LIKE ? OR w.title LIKE ? OR bc.title LIKE ?)';
        $like = '%' . $searchQ . '%';
