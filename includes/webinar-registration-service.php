...
/** @return array{full_name: string, email: string, mobile: string, city: string, organization: string}|null */
...
function ensureWebinarIntakeForConfirm(int $webinarId): ?array
{
    ...
    $result = saveWebinarRegistrationIntake($webinarId, $profile, $loggedInUserId);
    if (!$result['success']) {
        return fetchPendingWebinarIntakeByEmail($webinarId, $profile['email']);
    }

    return fetchWebinarIntakeByToken((string) ($result['token'] ?? ''), $webinarId);
}
...
function webinarEmailAlreadyRegistered(int $webinarId, string $email): bool
{
    ...
    if (db()->fetchOne(
        'SELECT wr.id FROM webinar_registrations wr INNER JOIN users u ON u.id = wr.user_id
         WHERE wr.webinar_id = ? AND LOWER(u.email) = ? LIMIT 1',
        [$webinarId, $email]
    )) {
        return true;
    }
    ...
    return (bool) db()->fetchOne(
        "SELECT id FROM webinar_registration_intakes
         WHERE webinar_id = ? AND LOWER(email) = ? AND status IN ('pending','confirmed') LIMIT 1",
        [$webinarId, $email]
    );
}
...
function completeFreeWebinarRegistrationFromIntake(string $token, int $webinarId): array
{
    ...
    $existing = db()->fetchOne(
        'SELECT id, registration_no FROM webinar_registrations WHERE user_id = ? AND webinar_id = ?',
        [$userId, $webinarId]
    );
    if ($existing) {
        db()->execute(
