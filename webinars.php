...
$webinars = $detailWebinar ? [] : publicFetchWebinars();
$pageTitle = $detailWebinar ? (string) ($detailWebinar['title'] ?? 'Webinar') : 'Webinars';
$navActive = 'webinars';
?>
<!DOCTYPE html>
<html lang="en">
...
<?php if ($detailWebinar):
    $w = $detailWebinar;
    $thumbUrl = catalogImageUrl(webinarThumbPathForTitle((string) ($w['title'] ?? '')));
...
<header class="page-hero webinar-detail-hero">
  <div class="container text-start" style="max-width:820px">
    ...
    <div class="webinar-thumb-wrap mb-3">
      <img class="webinar-thumb" src="<?= $thumbUrl ?>" alt="<?= htmlspecialchars((string) ($w['title'] ?? 'Webinar')) ?>" loading="lazy">
    </div>
    <div class="d-flex flex-wrap gap-2 align-items-center mb-2">
      <span class="webinar-badge <?= $badgeClass ?>">
        <?php if (($w['status'] ?? '') === 'live'): ?>Live Now
        <?php elseif (!empty($w['is_free'])): ?>Free
        <?php else: ?><?= htmlspecialchars(webinarPaidCompactLabel()) ?><?php endif; ?>
      </span>
      <?php if (!empty($w['category'])): ?>
      <span style="font-size:.8rem;color:var(--cyber-muted)"><?= htmlspecialchars((string) $w['category']) ?></span>
      <?php endif; ?>
    </div>
    <h1><?= htmlspecialchars((string) $w['title']) ?></h1>
    ...
    <p style="color:#fff;line-height:1.75;margin-bottom:1rem"><?= nl2br(htmlspecialchars((string) $w['description'])) ?></p>
    <?php elseif (!empty($w['short_desc'])): ?>
    <p style="color:#fff;line-height:1.75;margin-bottom:1rem"><?= htmlspecialchars((string) $w['short_desc']) ?></p>
    <?php endif; ?>
    <?php if (!empty($w['instructor'])): ?>
    <div class="webinar-instructor mb-3">
      <div class="instructor-avatar"><?= strtoupper(substr((string) $w['instructor'], 0, 1)) ?></div>
      <span><?= htmlspecialchars((string) $w['instructor']) ?></span>
    </div>
    <?php endif; ?>
  ...
  <div class="container">
    ...
    <div class="row g-4">
      <?php foreach ($webinars as $w):
        $thumbRel = webinarThumbPathForTitle((string) ($w['title'] ?? ''));
        $thumbUrl = catalogImageUrl($thumbRel);
        $fillPct = (int) $w['max_seats'] > 0
            ? min(100, ((int) $w['registered_seats'] / (int) $w['max_seats']) * 100)
            : 0;
        $seatsLeft = max(0, (int) $w['max_seats'] - (int) $w['registered_seats']);
      ...
      <div class="col-md-6 col-lg-4 d-flex">
        <div class="webinar-card">
          <div class="webinar-card-body">
            <div class="webinar-thumb-wrap">
              <img
                class="webinar-thumb"
                src="<?= $thumbUrl ?>"
                alt="<?= htmlspecialchars((string) ($w['title'] ?? 'Webinar')) ?> thumbnail"
                loading="lazy"
                decoding="async"
                data-lightbox-src="<?= $thumbUrl ?>"
                data-lightbox-title="<?= htmlspecialchars((string) ($w['title'] ?? 'Webinar')) ?>"
              >
            </div>
            <div class="d-flex justify-content-between align-items-start mb-2">
              <span class="webinar-badge <?= $badgeClass ?>">
                <?php if ($w['status'] === 'live'): ?>Live Now
                <?php elseif (!empty($w['is_free'])): ?>Free
                <?php else: ?><?= htmlspecialchars(webinarPaidCompactLabel()) ?>
              <?php endif; ?>
              </span>
              <?php if (!empty($w['category'])): ?>
            ...
            <h3 class="webinar-title">
              <a href="<?= url('webinars.php?slug=' . rawurlencode((string) ($w['slug'] ?? ''))) ?>" style="color:inherit">
          ...
          <div class="webinar-card-footer">
            <?php renderWebinarCardRegisterCta($w); ?>
          </div>
        </div>
      </div>
      <?php endforeach; ?>
