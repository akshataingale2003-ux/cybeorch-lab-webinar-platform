...
function assignmentEnsureUniqueSlug(string $slug, int $excludeId = 0): string
{
    $base = $slug !== '' ? $slug : 'assignment';
    $candidate = $base;
    $suffix = 1;

    while (true) {
        $params = [$candidate];
        $sql = 'SELECT id FROM assignments WHERE slug = ?';
        if ($excludeId > 0) {
            $sql .= ' AND id != ?';
            $params[] = $excludeId;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($routeEditId > 0 && $postId > 0 && $postId !== $routeEditId) {
        adminFlashRedirect('admin/assignments.php', 'error', 'Invalid update request. Please reload and try again.');
    }
    $data = [
        ...
        'project_type'     => CATALOG_PROJECT_TYPE_HANDS_ON,
    ...
    $data['slug'] = assignmentEnsureUniqueSlug((string) $data['slug'], $postId);
    ...
    if ($postId > 0) {
        db()->execute(
            'UPDATE assignments SET title=?,slug=?,short_desc=?,description=?,category=?,type=?,skills=?,duration=?,mode=?,status=?,apply_route=?,project_type=?,show_on_homepage=?,sort_order=?,updated_at=NOW() WHERE id=?',
    ...
    db()->execute(
        'INSERT INTO assignments (title,slug,short_desc,description,category,type,skills,duration,mode,status,apply_route,project_type,show_on_homepage,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        array_values($data)
    );
      ...
      <div class="row g-3">
        ...
        <div class="col-12">
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="show_on_homepage" value="1" id="assignmentShowOnHomepage" <?= !empty($editRow['show_on_homepage']) ? 'checked' : '' ?>>
            <label class="form-check-label" for="assignmentShowOnHomepage">Show on homepage / landing page</label>
          </div>
          <p class="form-text" style="font-size:.75rem;color:var(--cyber-muted)">Project type: <strong>Hands-On Project</strong>. Appears on the Hands-On Projects page when open. Homepage display is optional.</p>
        </div>
