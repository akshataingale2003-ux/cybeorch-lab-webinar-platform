...
function liveProjectEnsureUniqueSlug(string $slug, int $excludeId = 0): string
{
    $base = $slug !== '' ? $slug : 'live-project';
    $candidate = $base;
    $suffix = 1;

    while (true) {
        $params = [$candidate];
        $sql = 'SELECT id FROM live_projects WHERE slug = ?';
        if ($excludeId > 0) {
            $sql .= ' AND id != ?';
            $params[] = $excludeId;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($routeEditId > 0 && $postId > 0 && $postId !== $routeEditId) {
        adminFlashRedirect('admin/live-projects.php', 'error', 'Invalid update request. Please reload and try again.');
    }
    ...
    $data = [
        ...
        'project_type'     => CATALOG_PROJECT_TYPE_LIVE,
        'show_on_homepage' => catalogParseShowOnHomepage($_POST['show_on_homepage'] ?? null),
    ...
    $data['slug'] = liveProjectEnsureUniqueSlug((string) $data['slug'], $postId);
    ...
    if ($postId > 0) {
        db()->execute(
            'UPDATE live_projects SET title=?,slug=?,short_desc=?,description=?,category=?,icon_class=?,card_theme=?,image_path=?,features_json=?,stack=?,duration=?,team_size=?,status=?,project_type=?,show_on_homepage=?,sort_order=?,updated_at=NOW() WHERE id=?',
    ...
    db()->execute(
        'INSERT INTO live_projects (title,slug,short_desc,description,category,icon_class,card_theme,image_path,features_json,stack,duration,team_size,status,project_type,show_on_homepage,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        array_values($data)
    );
      ...
      <div class="row g-3">
        ...
        <div class="col-12">
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="show_on_homepage" value="1" id="liveShowOnHomepage" <?= !empty($editRow['show_on_homepage']) ? 'checked' : '' ?>>
            <label class="form-check-label" for="liveShowOnHomepage">Show on homepage / landing page</label>
          </div>
          <p class="form-text" style="font-size:.75rem;color:var(--cyber-muted)">Project type: <strong>Live Project</strong>. Appears on the Live Projects page when active. Homepage display is optional.</p>
        </div>
        <div class="col-12">
