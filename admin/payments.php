...
<div class="section-card">
  ...
  <div class="section-card-body">
    <div class="detail-grid">
      ...
      <div class="detail-item"><label>Receipt No.</label><span><code><?= htmlspecialchars($viewPayment['receipt_no'] ?? $viewPayment['invoice_no'] ?: '—') ?></code></span></div>
      <div class="detail-item"><label>Invoice</label><span><?= htmlspecialchars($viewPayment['invoice_no'] ?: '—') ?></span></div>
      <?php if (in_array(strtolower((string) ($viewPayment['status'] ?? '')), ['paid', 'refunded'], true)): ?>
      <div class="detail-item"><label>Payment slip</label><span>
        <a href="<?= htmlspecialchars(paymentSlipUrl((int) $viewPayment['id'], 'html')) ?>" class="btn-sm-link" target="_blank" rel="noopener"><i class="fas fa-eye"></i> View</a>
        <a href="<?= htmlspecialchars(paymentSlipUrl((int) $viewPayment['id'], 'pdf')) ?>" class="btn-sm-link ms-2"><i class="fas fa-file-pdf"></i> Download PDF</a>
      </span></div>
      <?php endif; ?>
...
<div class="section-card">
  ...
  <div class="section-card-body">
    ...
    $renderPaymentActions = static function (array $p) use ($page): void {
      ...
      <?php if (in_array(strtolower((string) ($p['status'] ?? '')), ['paid', 'refunded'], true)): ?>
      <a href="<?= htmlspecialchars(paymentSlipUrl($id, 'html')) ?>" class="btn-sm-cyber" target="_blank" rel="noopener"><i class="fas fa-receipt"></i><span>Slip</span></a>
      <a href="<?= htmlspecialchars(paymentSlipUrl($id, 'pdf')) ?>" class="btn-sm-cyber"><i class="fas fa-file-pdf"></i><span>PDF</span></a>
      <?php endif; ?>
