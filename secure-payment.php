...
$paidWebinarTracks = [
    'cybersecurity' => [
        ...
        'fee' => 2999, 'duration' => '120 min', 'schedule' => '07 Jun 2026 · 11:00 AM', 'instructor' => 'Priya Nair',
    ],
    'ai-ml' => [
        'title' => 'AI / ML', 'desc' => 'Machine learning, AI tooling, and production automation.',
        'fee' => 2999, 'duration' => '120 min', 'schedule' => '20 Jun 2026 · 11:00 AM', 'instructor' => 'Priya Nair',
    ],
    'blockchain' => [
        'title' => 'Blockchain', 'desc' => 'Web3, smart contracts, and wallet security.',
        'fee' => 2999, 'duration' => '120 min', 'schedule' => '27 Jun 2026 · 11:00 AM', 'instructor' => 'Priya Nair',
    ],
    'devops' => [
...
$paidBootcamps = array_values(array_filter(
    ...
    static fn (array $b): bool => (float) ($b['discounted_fee'] ?? 0) > 0
));
...
if ($selectedSlug === '' && $selectedType === 'webinar') {
    $selectedSlug = 'ai-ml';
}
...
if ($selectedType === 'bootcamp' && $selectedId > 0) {
    $row = publicFetchBootcampById($selectedId);
} elseif ($selectedType === 'bootcamp' && $selectedSlug !== '') {
    $row = publicFetchBootcampBySlug($selectedSlug);
} elseif ($selectedType === 'webinar' && $selectedId > 0) {
    $row = publicFetchWebinarById($selectedId);
} else {
    $row = null;
}
...
if (!empty($row)) {
    ...
    $programMeta = $selectedType === 'bootcamp'
        ? [
            ...
            'Schedule' => date('d M Y', strtotime((string) $row['start_date'])) . ' – ' . date('d M Y', strtotime((string) $row['end_date'])),
            'Instructor' => (string) ($row['instructor'] ?? ''),
        ]
        : [
            'Category' => (string) ($row['category'] ?? ''),
            'Duration' => ((int) ($row['duration_mins'] ?? 0)) . ' min',
            'Schedule' => date('d M Y, h:i A', strtotime((string) $row['scheduled_at'])),
            'Instructor' => (string) ($row['instructor'] ?? ''),
        ];
} elseif ($selectedType === 'bootcamp' && isCybeorchProgramPathSlug($selectedSlug)) {
    $cfg = cybeorchProgramPathCatalog()[$selectedSlug];
    ...
    $programLabel = (string) $cfg['title'];
    $programFee = bootcampCheckoutFee($row, $paymentCurrency);
    $programDesc = (string) $cfg['short_desc'];
    $programMeta = [
        'Category' => (string) $cfg['category'],
        'Duration' => bootcampDurationDisplay($row),
        'Instructor' => (string) $cfg['instructor'],
    ];
...
$bootcampPricingRow = ($selectedType === 'bootcamp' && !empty($row))
...
<meta name="description" content="Secure payment for CYBEORCH LAB webinars and bootcamps.">
      ...
      <div class="col-lg-5">
        <aside class="course-summary">
          ...
          <div class="amount-box">
            <div class="amount-label"><?= $selectedType === 'webinar' && $programFee > 0 ? htmlspecialchars(webinarPaidCompactLabel()) : ('Total amount (' . (($selectedType === 'webinar' || $bootcampDualCurrency) ? htmlspecialchars($paymentCurrency) : 'INR') . ')') ?></div>
            <div class="amount-value" id="summaryAmountValue"><?= htmlspecialchars($selectedType === 'webinar' ? webinarFormatCheckoutAmount($paymentCurrency, $programFee) : $bootcampDisplayAmount) ?></div>
            <?php if ($bootcampDualCurrency): ?>
            <div class="bootcamp-dual-price-summary" id="summaryDualPrice" style="font-size:.88rem;color:var(--cyber-muted);margin-top:.5rem;line-height:1.5">
              <div><?= htmlspecialchars($bootcampAmountInr) ?></div>
              <div>(<?= htmlspecialchars($bootcampAmountUsd) ?>)</div>
            </div>
            <?php endif; ?>
            <div style="font-size:.78rem;color:var(--cyber-muted);margin-top:.35rem" id="summaryAmountNote"><?= ($selectedType === 'webinar' || $bootcampDualCurrency) ? ($paymentCurrency === 'INR' ? 'Inclusive of all taxes' : 'Fixed USD price — no conversion') : 'GST included · INR' ?></div>
          </div>
          ...
          <form method="POST" action="<?= url('secure-payment.php') ?>" id="securePayForm">
            ...
            <select class="form-select mb-4" id="programPicker" aria-label="Select program">
              <optgroup label="Webinar tracks">
                ...
                <option value="webinar:<?= htmlspecialchars($slug) ?>"<?= $selectedType === 'webinar' && $selectedSlug === $slug ? ' selected' : '' ?>>
                  <?= htmlspecialchars($t['title']) ?> — <?= htmlspecialchars(webinarPaidOptionPriceLabel()) ?>
                </option>
                <?php endforeach; ?>
              </optgroup>
              ...
              <optgroup label="Scheduled webinars">
                ...
                <option value="webinar-id:<?= (int) $w['id'] ?>"<?= $selectedId === (int) $w['id'] ? ' selected' : '' ?>>
                  <?= htmlspecialchars($w['title']) ?> — <?= htmlspecialchars(webinarPaidOptionPriceLabel()) ?>
                </option>
                <?php endforeach; ?>
              </optgroup>
              <?php endif; ?>
              ...
              <optgroup label="Bootcamps">
                ...
                ?>
                <option value="bootcamp-id:<?= (int) $b['id'] ?>"<?= $selectedType === 'bootcamp' && $selectedId === (int) $b['id'] ? ' selected' : '' ?>>
            ...
            <?php if ($selectedType === 'webinar' || $bootcampDualCurrency): ?>
            ...
            <div class="pay-step-label<?= ($selectedType === 'webinar' || $bootcampDualCurrency) ? ' mt-4' : '' ?>">Step <?= ($selectedType === 'webinar' || $bootcampDualCurrency) ? '3' : '2' ?> — Your details</div>
...
(function () {
  ...
  var isBootcampDual = <?= $bootcampDualCurrency ? 'true' : 'false' ?>;
  var webinarAmounts = { INR: <?= json_encode(webinarFormatCheckoutAmount('INR', (float) WEBINAR_PAID_FEE_INR)) ?>, USD: <?= json_encode(webinarFormatCheckoutAmount('USD', (float) WEBINAR_PAID_FEE_USD)) ?> };
  ...
  function syncPayCurrencyUi() {
    if (!isWebinar && !isBootcampDual) return;
    ...
    if (summaryNote) summaryNote.textContent = code === 'INR' ? 'Inclusive of all taxes' : 'Fixed USD price — no conversion';
  ...
  document.getElementById('programPicker')?.addEventListener('change', function () {
    var v = this.value || '';
    var cur = (isWebinar || isBootcampDual) ? '&currency=' + encodeURIComponent(selectedPayCurrency()) : '';
