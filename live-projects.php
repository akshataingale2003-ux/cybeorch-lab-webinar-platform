...
$pageTitle = $isDetail ? (string) $project['title'] : 'Live Projects';
...
<title><?= htmlspecialchars($pageTitle) ?><?= $isDetail ? ' – Live Projects' : '' ?> – CYBEORCH LAB</title>
<?php if ($isDetail): ?>
<meta name="description" content="<?= htmlspecialchars((string) ($project['short_desc'] ?? $project['description'] ?? 'Live project at CYBEORCH LAB')) ?>">
<?php else: ?>
<meta name="description" content="Explore CYBEORCH live project tracks — Cyber Security, Java, MERN, Data Science, AI & ML, and FinTech with NxL credits.">
<?php endif; ?>
...
<style>
  ...
  .lp-premium-container::before{
    content:"";
    position:absolute;
    inset:-2px;
    background:
      linear-gradient(120deg, rgba(0,212,255,0.22), rgba(0,255,136,0.14), rgba(255,107,53,0.10));
    opacity:.18;
    pointer-events:none;
    filter:blur(18px);
  }
  .lp-premium-container > *{position:relative;z-index:1}
  .lp-premium-container:hover{
    transform:translateY(-2px);
    border-color:rgba(0,212,255,0.42);
    box-shadow:0 26px 92px rgba(0,212,255,0.10), 0 22px 80px rgba(0,0,0,0.55);
  }
  .lp-premium-head{margin-bottom:clamp(1rem,2.5vw,1.6rem)}
  .lp-premium-footer{margin-top:clamp(1.25rem,3vw,2rem);padding-top:clamp(1rem,2.5vw,1.35rem);border-top:1px solid rgba(0,212,255,0.14)}
...
<?php if ($isDetail):
    ...
    $fallbackUrl = lpAssetUrl('assets/images/AI & ML.png');
?>
...
<section class="section pt-0">
  <div class="container">
    <div class="lp-premium-container">
      <div class="text-center lp-premium-head">
        <h2 class="section-title">Project <span class="accent">Categories</span></h2>
        <div class="divider"></div>
        <p style="color:var(--cyber-muted);max-width:720px;margin:0 auto;line-height:1.75">Each track is managed from the admin dashboard and updated dynamically. Join as a contributor, intern, or collaborator.</p>
      </div>
      ...
      <div class="row g-4">
        <?php foreach ($projects as $p):
          ...
          $title = (string) ($p['title'] ?? 'Live Project');
      ...
      <div class="text-center lp-premium-footer">
