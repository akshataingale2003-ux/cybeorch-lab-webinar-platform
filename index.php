...
// IMPORTANT: Fix Live Webinar Seat Count Logic
// Seats booked must always be computed from actual webinar_registrations rows (no caching).
$webinarCategories = publicFetchWebinars(4);
...
<title>CYBEORCH LAB &ndash; Cybersecurity Education &amp; Software Development Company</title>
...
/* CYBEORCH LAB + Services banner spacing */
...
.badge-paid { background: rgba(255, 193, 7, 0.15); color: var(--cyber-gold-dark); border: 1px solid rgba(255, 193, 7, 0.3); text-transform: none; letter-spacing: 0.02em; }
...
<section class="section" id="webinars">
  <div class="container">
    ...
    <div class="row g-4">
      <?php foreach ($webinarCategories as $cat):
        $scheduledAt = (string) ($cat['scheduled_at'] ?? '');
        $seatsLeft = max(0, (int) $cat['max_seats'] - (int) $cat['registered_seats']);
        $fillPct = (int) $cat['max_seats'] > 0
            ? min(100, ((int) $cat['registered_seats'] / (int) $cat['max_seats']) * 100)
            : 0;
        $thumbUrl = catalogImageUrl(webinarThumbPathForTitle((string) ($cat['title'] ?? '')));
      ?>
      <div class="col-md-6 col-lg-3 fade-in d-flex">
          ...
          <div class="webinar-card-body">
            <div class="webinar-thumb-wrap">
              <img
                class="webinar-thumb"
                src="<?= $thumbUrl ?>"
                alt="<?= htmlspecialchars($cat['title'], ENT_QUOTES, 'UTF-8') ?> — CYBEORCH webinar"
                loading="lazy"
                decoding="async"
              >
            </div>
            <div class="d-flex justify-content-between align-items-start mb-2">
              <span class="webinar-badge <?= !empty($cat['is_free']) ? 'badge-free' : 'badge-paid' ?>">
                <?= htmlspecialchars(webinarCardStatusLabel($cat)) ?>
              </span>
            </div>
            <h3 class="webinar-title"><?= htmlspecialchars((string) ($cat['title'] ?? '')) ?></h3>
            <p class="webinar-desc"><?= htmlspecialchars((string) ($cat['short_desc'] ?? '')) ?></p>
            ...
            <div class="webinar-instructor">
              <div class="instructor-avatar"><?= htmlspecialchars(strtoupper(substr((string) ($cat['instructor'] ?? 'C'), 0, 1))) ?></div>
              <span class="instructor-name"><?= htmlspecialchars((string) ($cat['instructor'] ?? '')) ?></span>
            </div>
            <div class="d-flex justify-content-between align-items-center mb-1" style="font-size:0.8rem; color:var(--cyber-muted)">
              <span><?= (int) $cat['registered_seats'] ?> registered</span>
...
renderAuthRegistrationPopup(buildAuthRegistrationPopupConfig([
    ...
    'success_url'            => url('index.php?registered=1'),
    'ref_code'               => $refCode,
